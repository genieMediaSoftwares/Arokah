...
function AddEvent() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title || !imageURL) { toast.error("Event title and banner image are required"); return; }
    setSubmitting(true);
    try {
      await createEvent({
        ...
        // createdAt / updatedAt are set by the server so the timestamps can't
            ...
            <Section number="05" title="Event Image" description="Upload the main event banner image. A high-resolution horizontal photo works best.">
              <div>
                ...
                <ImageUploader
                  ...
                  folder="events"
                  aspect="aspect-[16/9]"
                  hint="This is the image shown on the events listing and the event page."
                />
              </div>
            </Section>
